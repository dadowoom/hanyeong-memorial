# 한영교회 역사관 작업 인수인계 지시문

> 운영 정비와 배포는 [운영 절차](./OPERATIONS.md)를 먼저 확인한다.
> 아래의 main 직접 반영·서버 `git reset --hard`·일반 PM2 재시작 예시는 과거 배포 흐름의 기록이다.
> 현재 서버의 수정 파일을 보존하고, 검토된 커밋과 한영 전용 재시작 도구로 명시적으로 배포한다.
> 2026-09-08 전용 재시작 도구의 root 설치와 실제 호출 검증을 완료했다. 앱은 `hanyeongapp` UID/GID 997로 실행한다.
> 기존 환경 34개·배포 파일 102개 보존을 확인했으며, 운영 앱은 `e02e0c6`을 유지한다. 저장소 main 갱신을 새 앱 배포로 해석하지 않는다.
> 2026-09-08 예약 백업 `20260908-051001`은 로컬·암호화 외부 보관본의 내용 일치, DB 12개 테이블과 정상 종료 표식·압축 무결성을 확인했다. 실제 DB 복원 리허설을 새로 수행한 기록은 아니며, 상세 결과는 위 운영 절차를 따른다.
...
- 교회 연혁은 빈 데이터로 시작하며 관리자가 연대와 기록을 직접 등록
- 다른 교회의 연혁 데이터는 이식하지 않음
